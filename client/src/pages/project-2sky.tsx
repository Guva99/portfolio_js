...
import { ExternalLink, Smartphone, Layout, FileText } from "lucide-react";
...
export default function Project2Sky() {
  ...
  const responsibilities = t("projectPage.connectivity.responsibilities", { returnObjects: true }) as { label: string; text: string }[];
  const contributions = t("projectPage.connectivity.contributions", { returnObjects: true }) as { title: string; desc: string }[];
  const contribIcons = [Layout, FileText];
              ...
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <span className="px-3 py-1 rounded-full bg-blue-600/10 text-blue-400 text-sm font-medium border border-blue-600/20 flex items-center gap-1.5">
                    <Smartphone className="w-3.5 h-3.5" /> {t("projectPage.connectivity.badge")}
                  </span>
                </div>
              ...
              <div>
                ...
                >
                  <h2 className="text-3xl md:text-5xl font-bold mb-6">
                    {t("projectPage.connectivity.h2")} <br />
                    <span className="text-blue-500">{t("projectPage.connectivity.h2Highlight")}</span>
                  </h2>
                  
                  <p className="text-slate-300 text-lg leading-relaxed mb-8">
                    {t("projectPage.connectivity.description")}
                  </p>
                  
                  <h3 className="text-xl font-semibold text-white mb-4">{t("projectPage.myKeyContributions")}</h3>
                 ...
                 <div className="grid grid-cols-3 gap-4 h-full">
                    <div className="relative overflow-hidden h-full">
                       <motion.div animate={{ y: ["0%", "-50%"] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="flex flex-col gap-4 w-full">
                         {column1.map((img, i) => (<ImageWithShimmer key={i} src={img} alt={`App Screen ${i}`} className="w-full rounded-xl border border-white/10" shimmerColor="bg-blue-600/20" wrapperClassName="w-full rounded-xl" />))}
                       </motion.div>
                    </div>
                    <div className="relative overflow-hidden h-full">
                       <motion.div animate={{ y: ["-50%", "0%"] }} transition={{ duration: 25, repeat: Infinity, ease: "linear" }} className="flex flex-col gap-4 w-full">
                         {column2.map((img, i) => (<ImageWithShimmer key={i} src={img} alt={`App Screen ${i}`} className="w-full rounded-xl border border-white/10" shimmerColor="bg-blue-600/20" wrapperClassName="w-full rounded-xl" />))}
                       </motion.div>
                    </div>
                    <div className="relative overflow-hidden h-full">
                       <motion.div animate={{ y: ["0%", "-50%"] }} transition={{ duration: 22, repeat: Infinity, ease: "linear" }} className="flex flex-col gap-4 w-full">
                         {column3.map((img, i) => (<ImageWithShimmer key={i} src={img} alt={`App Screen ${i}`} className="w-full rounded-xl border border-white/10" shimmerColor="bg-blue-600/20" wrapperClassName="w-full rounded-xl" />))}
                       </motion.div>
                    </div>
                 </div>
              </div>
              ...
              <div className="md:col-span-2">
                 ...
                 <div className="space-y-6">
                    {contributions.map((c, i) => {
                      const Icon = contribIcons[i] || Layout;
                      return (
                        <div key={i} className="bg-slate-900/40 border border-white/5 rounded-xl p-6 hover:border-blue-600/30 transition-colors">
                          <div className="flex items-start gap-4">
                            <div className="p-3 rounded-lg bg-blue-600/10 text-blue-400 shrink-0"><Icon className="w-6 h-6" /></div>
                            <div>
